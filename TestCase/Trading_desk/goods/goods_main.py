...
from Params.params_goods import *
...
class TestGoods(object):
    ...
    @allure.description('启用/禁用课程')
    @pytest.mark.parametrize('case', OpenOrCloseGoods().case_data)
    def test_goods_03(self, case):
        log.info("*************** 开始执行用例 ***************")
        log.info("用例名称  ==>> {}".format(case['test_name']))
        result = notify().notify_result(case['mode'], url + case['url'], case['data'], header, case['type'])
    ...
    @allure.description('获取一个课程的详细信息(基本字段和动态字段)')
    @pytest.mark.parametrize('case', GetOneCourse().case_data)
    def test_goods_03(self, case):
        log.info("*************** 开始执行用例 ***************")
        log.info("用例名称  ==>> {}".format(case['test_name']))
        result = notify().notify_result(case['mode'], url + case['url'], case['data'], header, case['type'])
    ...
    @pytest.mark.parametrize('case', GetCourseList().case_data)
    def test_goods_03(self, case):
        log.info("*************** 开始执行用例 ***************")
        log.info("用例名称  ==>> {}".format(case['test_name']))
        result = notify().notify_result(case['mode'], url + case['url'], case['data'], header, case['type'])
